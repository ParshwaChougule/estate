...
function OffPlan({ statusFilter = 'all', searchFilters = {} }) {
...
🔗 Check out this amazing off-plan property!
    `.trim();
  ...
  useEffect(() => {
    // Fetch residential properties from Firebase
  ...
  return (
    <Container className="my-5">
      <Row className="mb-4 align-items-center">
        <Col>
          <h2 className="fw-bold section-title">Residential Properties</h2>
        </Col>
        <Col className="text-end">
          {filteredProperties.length > 3 && (
            <Button 
              variant="dark" 
              onClick={() => navigate('/residential-properties')}
            >
            ...
            <Col className="text-center py-5">
              <p className="text-muted">
                {searchFilters.location || searchFilters.priceRange || searchFilters.bedrooms 
                  ? `No residential properties found matching your search criteria.`
